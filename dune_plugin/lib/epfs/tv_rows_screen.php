...
class Tv_Rows_Screen extends Abstract_Rows_Screen
{
    const ID = 'rows_epf';
        ...
        foreach ($groups as $group) {
            ...
            for ($i = 0, $iMax = count($items); $i < $iMax; $i += PaneParams::max_items_in_row) {
                $rows[] = Rows_Factory::regular_row(
                    ...
                    $group->get_title(),
                    $row_gid,
                    RowsParams::width, RowsParams::height,
    ...
    {
        //hd_print('Tv_Rows_Screen: handle_user_input:');
        ...
        switch ($control_id) {
            case GUI_EVENT_TIMER:
                if ($this->need_update_epf_mapping_flag) {
            ...
            case PLUGIN_FAVORITES_OP_MOVE_DOWN:
                if (($media_url->group_id !== Default_Dune_Plugin::FAV_CHANNEL_GROUP_ID)
                    && (strpos('_' . PLUGIN_FAVORITES_OP_REMOVE . PLUGIN_FAVORITES_OP_MOVE_UP . PLUGIN_FAVORITES_OP_MOVE_DOWN, $control_id)))
                    return null;
