...
class vidok_config extends default_config
{
    ...
    public function GetAccountInfo($force = false)
    {
        ...
        try {
            ...
            if ($force !== false || empty($this->account_data)) {
                ...
                $this->account_data = Curl_Wrapper::decodeJsonResponse(false, $this->setup_curl()->download_content($url));
    ...
    protected function save_settings($param)
    {
        try {
            ...
            $content = Curl_Wrapper::decodeJsonResponse(false, $this->setup_curl()->download_content($url), true);
