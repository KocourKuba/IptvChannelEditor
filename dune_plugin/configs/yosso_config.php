...
class yosso_config extends default_config
{
    const PAGE_LIMIT = 50;
    ...
    public function GetAccountInfo($force = false)
    {
        ...
        $this->jfc->init($this->GetVodListUrl(), $this->plugin_info['app_version']);
        hd_debug_print("Collect information from account: " . var_export($force, true));
        $this->account_data = $this->jfc->login($this->get_login(), $this->get_password());
    ...
    public function TryLoadMovie($movie_id)
    {
        ...
        if ($movie_type === jellyfin_api::MOVIES) {
            ...
            $movie_series = new Movie_Series($real_id, $name, $this->jfc->getPlayUrl($real_id));
            $movie->add_series_data($this->fill_series($movie_series, $real_id, safe_get_value($movie_item, 'MediaSources', array())));
            $qualities_str = implode(', ', array_diff($movie->qualities_list, array('auto')));
            ...
            foreach (safe_get_value($seasons, 'Items', array()) as $season) {
                ...
                foreach (safe_get_value($episodes, 'Items', array()) as $episode) {
                    ...
                    $movie_series = new Movie_Series($episode_id, TR::t('vod_screen_series__1',  safe_get_value($episode, 'Name', 'no name')),
    ...
    public function fetchVodCategories(&$category_list, &$category_index)
    {
        ...
        foreach ($collections as $collection) {
            ...
            $collection_type = safe_get_value($collection, 'CollectionType', jellyfin_api::MOVIES);
            hd_debug_print("Collection type: $collection_type");
            if ($collection_type === jellyfin_api::TVSHOWS) {
                $sid .= '_' . jellyfin_api::SERIES;
            } else if ($collection_type === jellyfin_api::MOVIES) {
                $sid .= '_' . jellyfin_api::MOVIES;
            } else {
                hd_debug_print("Unsupported collection type: $collection_type");
                continue;
            }
        ...
        hd_debug_print("Categories read: " . count($category_index));
        ...
        foreach ($media_sources as $source) {
            ...
            foreach (safe_get_value($source, 'MediaStreams', array()) as $stream) {
                if (strcasecmp(safe_get_value($stream, 'Type'), 'Video') === 0) {
                    ...
                    $quality = new Movie_Variant($name, $name, $this->jfc->getDownloadUrl($stream_id));
                    // default playback url for quality
                    if ($stream_id == $real_id) {
                        $movie_series->qualities['auto'] = new Movie_Variant('auto', 'auto', $this->jfc->getDownloadUrl($stream_id));
                    }
                    $movie_series->qualities[$name] = $quality;
                    break;
                }
            }
        }
    ...
    protected function CreateShortMovie($movie_info, &$movies)
    {
        ...
        $movies[] = new Short_Movie("{$id}_$type", $name, $icon, TR::t('vod_screen_movie_info__2', $name, $rating));
    }
}
