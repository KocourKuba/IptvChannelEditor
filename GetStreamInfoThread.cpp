...
void CGetStreamInfoThread::GetChannelStreamInfo(const CString& probe, const std::string& url, std::string& audio, std::string& video)
{
	if (url.empty())
		return;
	...
	CString csCommand;
	csCommand.Format(_T("\"%s\" -hide_banner -show_streams \"%hs\""), probe.GetString(), url.c_str());
