...
- *Редактор*
    ...
    - Кнопка '+' копирует текущий список каналов в новый создаваемый список
    - Добавлена расцветка для измененных полей (epg, archive, icon).
    - Полная переработка для настроек плагина. Основные настройки вынесены в файл config.json
    - Добавлен просмотр и сохранение текущих настроек плагина. Подготовка к реализации Custom Plugin.
      Редактирование будет позже. Пока ручками в готовом плагине на свой страх и риск.
...
- *Плагины*
    ...
    - Универсализация плагина. Основные настройки теперь передаются через config.json
